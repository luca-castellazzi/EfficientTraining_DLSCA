...
def main():
    ...
    train_dev = sys.argv[4].upper() # Single train device
    ...
    for i in tqdm(range(n_start_configs, len(constants.KEYS))): 
        ...
        if start_key == end_key:
            config_label = f'{train_dev}_{start_key}'
        else:
            config_label = f'{train_dev}_{start_key}-{end_key.replace("K", "")}'
    ...
    # Save GEs as .CSV
    ge_file_path = constants.RESULTS_PATH + f'/1d/ge-{train_dev}vs{test_dev}.svg'
    ges_to_csv(ges, config_labels, ge_file_path)
    
    # Plot GEs
    ge_plot_path = constants.RESULTS_PATH + f'/1d/ge-{train_dev}vs{test_dev}.svg'
    ge_title = f'GE {train_dev}vs{test_dev}'
    ...
    s_title = f'GE {train_dev}vs{test_dev}'
    s_plot_path = constants.RESULTS_PATH + f'/1d/scores-{train_dev}vs{test_dev}.svg'
