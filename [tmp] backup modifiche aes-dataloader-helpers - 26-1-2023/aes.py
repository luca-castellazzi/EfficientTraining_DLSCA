...
# Int-version of Inverse AES SBox
...
def _add_round_key(p, k):
    # AddRoundKey
    sbox_in = p ^ k
    return sbox_in

def _sub_bytes(sbox_in):
    # SubBytes
    row, col = to_coords(sbox_in)
    sbox_out = SBOX_DEC[row, col]
    return sbox_out

def _inverse_sub_bytes(sbox_out):
    # SubBytes
    row, col = to_coords(sbox_out)
    sbox_in = INV_SBOX_DEC[row, col]
    return sbox_in
...
def forward_sbox_in(p, k):
    sbox_in = _add_round_key(p, k)
    return sbox_in

def forward_sbox_out(p, k):
    # AddRoundKey
    sbox_in = _add_round_key(p, k)
    # SubBytes
    sbox_out = _sub_bytes(sbox_in)
    return sbox_out

def forward_hw_sbox_out(p, k):
    # AddRoundKey
    sbox_in = _add_round_key(p, k)
    # SubBytes
    sbox_out = _sub_bytes(sbox_in)
...
def backward_key(l, p):
    # l is the key
    k = l
    return k

def backward_sbox_in(l, p):
    # l is the sbox_in
    k = _add_round_key(l, p) # The inverse of a XOR is the XOR itself
    return k

def backward_sbox_out(l, p):
    # l is the sbox_out
    sbox_in = _inverse_sub_bytes(l)
    k = _add_round_key(sbox_in, p) # The inverse of a XOR is the XOR itself
    return k

def backward_hw_sbox_out():
    pass
